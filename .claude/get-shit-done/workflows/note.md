...
- **Global scope**: `E:/tricio_s_application/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/tricio_s_application/.claude/notes/`)
...
2. Glob `E:/tricio_s_application/.claude/notes/*.md` (if directory exists) — global notes
...
Global (E:/tricio_s_application/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/tricio_s_application/.claude/notes/` — works in any directory
